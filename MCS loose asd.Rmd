...
########## less loose sample, please don't run
```{r eval=FALSE}
library(dplyr)
complete_asd <- subset(mcs_asd,!grepl("NA.*NA", pattern))
   # To filter out those with more than one NA
complete_asd <- complete_asd %>%
  mutate(
    num_diag = rowSums(dplyr::select(.,ends_with("AUTS00")) == 1, na.rm = TRUE),
    num_na = rowSums(is.na(dplyr::select(., ends_with("AUTS00")))))

asd_sample <- complete_asd %>% 
...
# Create a column to identify if it has at least two consecutive 1's
 asd_sample_diag <- asd_sample_diag %>% mutate(cons.diag = as.integer(grepl("1-1", pattern))) # N =  552
```
